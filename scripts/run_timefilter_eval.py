"""TimeFilter RAG evaluation: vanilla retrieval + year-scoped hard filter.

Fills the 2×2 ablation cell "temporal filter without KG":

                 │   No KG          │   + KG
    ─────────────┼──────────────────┼───────────────────
     No Temporal │   Vanilla RAG    │   KG²RAG
        + Temporal│   **TimeFilter** │   TempoRAG-KG

Same 7-model matrix and 129 QAs as `run_vanilla_eval.py`; only the retriever
differs. Uses `retrieve_with_year_filter(question, years=rec['years'])`.

Outputs land in `data/eval/timefilter/<model>/`.
"""
...
EVAL_OUT = REPO_ROOT / "data" / "eval" / "timefilter"
...
def _retrieve_all(
    ...
    *,
    ...
    """Year-filtered retrieval, shared across models. Records `fallback` when
    the filter produced fewer than k candidates and we reverted to vanilla —
    informative for diagnosing cases where the filter buys nothing."""
    ...
    for i, rec in enumerate(qa_records, 1):
        ...
        out[qid] = retrieve_with_year_filter(
            rec["question"], years, index, embed_client, cache, k=k,
        )
    ...
    with preds_path.open("w", encoding="utf-8") as f_out:
        for i, rec in enumerate(qa_records, 1):
            ...
            row = {
                ...
                "retrieved_fys": [c.get("fy") for c in chunks],
            }
...
def _write_report(summaries: list[dict]) -> None:
    ...
    lines = [
        ...
        "",
        "Retrieval: vanilla cosine + hard mask on `chunk.fy ∈ question.years`.",
        "Falls back to unfiltered retrieval if the mask yields < k candidates.",
        "",
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=f"Subset of configs. Default: all 7. Valid: {sorted(MODEL_REGISTRY)}",
    )
    ...
    print(f"Retrieving top-{args.k} year-filtered chunks for {len(qa_records)} questions ...")
